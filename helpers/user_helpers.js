...
module.exports={
    user_signup :(user_data)=>{
        return new Promise(async(resolve,reject)=>{
          let is_user_already_exist = await db.get().collection(collection.USER_COLLECTION).findOne({
               $or:[
           ...
           if(is_user_already_exist){
               ...
               let bcrypted_password = await bcrypt.hash(toString(user_data.passWord),10)
           let new_user ={
               name:user_data.name,
               email:user_data.email,
           ...
           db.get().collection(collection.USER_COLLECTION).insertOne(new_user).then((response)=>{
            ...
            let token = jwt.sign(user_token_data,process.env.JWT_SECRET,{expiresIn: 86400});
            // console.log(token);
...
login:(input)=>{
   ...
   var format = /[0-9]\w+/g;
    if(format.test(input)){
        // this input is phone
       let already_mobile_exist = await db.get().collection(collection.USER_COLLECTION).findOne({
           $and:[
               {phone:input},
               {auth:"password"},
               {status:"active"}
...
.create(
    ...
    //    console.log(token_user_data);
          let token =  jwt.sign(token_user_data,process.env.JWT_SECRET,{expiresIn: 86400})
        //   console.log(token);
...
bcrypt.compare(toString(data.password),user.password).then(status=>{
    if(status){
       ...
       let user_token =  jwt.sign(user_token_data,process.env.JWT_SECRET,{expiresIn:86400});
